...
class COGraphActivity : AppCompatActivity() {
    ...
    fun getDBReadings()
    {
        ...
        val deviceDetails: ResultSet = stmt.executeQuery("\n" +
                "SELECT TOP 20 ReadingPPM, DateRegistered FROM (SELECT TOP 20 ReadingPPM, DateRegistered FROM CO_Readings ORDER BY DateRegistered DESC) CO_Readings ORDER BY DateRegistered ASC");
